package cn.jwg.mycalendarmonthweek.calendar;
...
public class CalendarUtils {
    ...
    private Map<String, List<CalendarBean>> sMonthTaskHint = new HashMap<>();
    ...
    public List<CalendarBean> addTaskHints(int year, int month, List<CalendarBean> dataLists) {
        String key = hashKey(year, month);
        List<CalendarBean> hints = sUtils.sMonthTaskHint.get(key);
        if (hints == null) {
            hints = new ArrayList<>();
            hints.removeAll(dataLists); // 避免重复
            hints.addAll(dataLists);
    ...
    public List<CalendarBean> removeTaskHints(int year, int month, List<CalendarBean> dataLists) {
        String key = hashKey(year, month);
        List<CalendarBean> hints = sUtils.sMonthTaskHint.get(key);
        if (hints == null) {
            hints = new ArrayList<>();
            ...
            hints.removeAll(dataLists);
        }
        return hints;
    }

    public boolean addTaskHint(int year, int month, CalendarBean schedule) {
        String key = hashKey(year, month);
        List<CalendarBean> hints = sUtils.sMonthTaskHint.get(key);
        if (hints == null) {
            hints = new ArrayList<>();
    ...
    public boolean removeTaskHint(int year, int month, CalendarBean schedule) {
        String key = hashKey(year, month);
        List<CalendarBean> hints = sUtils.sMonthTaskHint.get(key);
        if (hints == null) {
            hints = new ArrayList<>();
            ...
            if (hints.contains(schedule)) {
                Iterator<CalendarBean> i = hints.iterator();
                while (i.hasNext()) {
                    CalendarBean sl = i.next();
                    if (sl.getDay() == schedule.getDay() && sl.getColor() == schedule.getColor()) {
                        i.remove();
                        break;
                    }
                }
    ...
    public List<CalendarBean> getTaskHints(int year, int month) {
        String key = hashKey(year, month);
        List<CalendarBean> hints = sUtils.sMonthTaskHint.get(key);
        if (hints == null) {
            hints = new ArrayList<>();
